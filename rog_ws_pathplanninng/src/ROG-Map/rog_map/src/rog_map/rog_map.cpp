...
ROGMap::ROGMap(const ros::NodeHandle& nh) :nh_(nh) {
    ...
    // 修复这里的错误：使用 string 变量保存时间消耗信息
    std::string time_columns;
...
bool ROGMap::evaluateCoarseEDT(const Eigen::Vector3d& pos, double& dist) {
    // 检查输入位置是否有效
    if (pos.array().isNaN().any() || pos.array().isInf().any()) {
        return false;
    }
    ...
    // 如果结果有效则返回true
    return !std::isnan(dist) && !std::isinf(dist);
}